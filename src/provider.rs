...
impl SproxProvider {
    pub fn new(data_directory: impl AsRef<Path>) -> Result<Self> {
        let data_directory = env::current_dir()?.join(data_directory).canonicalize()?;
        clean_old_tmp_files(&data_directory)?;
    ...
    pub(crate) fn resolve_abs_path(&self, path: impl AsRef<Path>) -> Result<PathBuf> {
        Ok(path
            .as_ref()
            .absolutize_virtually(&self.data_directory)?
            .into_owned())
    }
